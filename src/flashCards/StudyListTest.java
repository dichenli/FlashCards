...
public class StudyListTest {
    ...
    public final void testModify() {
        studylist.modify(item1, "2 + 3 ", " 5");
        assertEquals("2 + 3", item1.getStimulus()); //item1 is one element in studylist, so it will be modified
        assertEquals("5", item1.getResponse());
         
        //illegal arguments, item1 is not modified
        studylist.modify(item1, " ", " ");
        assertEquals("2 + 3", item1.getStimulus());
        assertEquals("5", item1.getResponse());
