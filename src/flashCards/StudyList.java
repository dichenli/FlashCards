/**
 * @author Dichen Li and Linjie Peng
 */
package flashCards;
...
public class StudyList {
    ...
    public void modify(Item item, String newStimulus, String newResponse) {
        ...
        for (int i = 0; i < this.getList().size(); i++) {
        	if (getList().get(i).getStimulus().equals(newStimulus)) {
        		throw new IllegalArgumentException("There already is a same stimulus here.");
        	}
        }
        for(Item i : items) {
        	if(i.getStimulus().trim().equals(item.getStimulus().trim()) || i.getResponse().trim().equals(item.getResponse().trim())) {
